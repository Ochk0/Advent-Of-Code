...
def stringToNum(s):
    ...
    return num_dict.get(s, "Invalid input")
def checkIfWord(s):
    for l in sNumbers:
        if l in s:
            return stringToNum(l)
    return False
def checkIfWordReversed(s):
    s = s[::-1]
    for l in sNumbers:
        if l in s:
            return stringToNum(l)
...
for word in words:
    ...
    for i, letter in enumerate(word):
        if letter in numbers and first == None:
            first = int(letter)
        if checkIfWord(word[0:i+1]) and first == None:
            first = checkIfWord(word[0:i+1])
        if checkIfWordReversed(word[wordlen:wordlen-i-1:-1]) and last == None:
            last = checkIfWordReversed(word[wordlen:wordlen-i-1:-1])
        if word[wordlen-i-1] in numbers and last == None:
            last = int(word[wordlen-i-1])
        if first and last:
            break
    sum+=first*10+last
print(sum)
