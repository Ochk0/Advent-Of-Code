a = """Game 1: 13 red, 18 green; 5 green, 3 red, 5 blue; 5 green, 9 red, 6 blue; 3 blue, 3 green
...
def splitNumbers(s):
    ...
    for i in s:
        ...
        number = q[index]
        color = q[index+1]
        # print(number, color, type(maxColor[color]))
        if int(number) > maxColor[color]:
            maxColor[color] = int(number)
    return maxColor['red'] * maxColor['green'] * maxColor['blue']
        
# print(getNumbers(b[0]))
for i, co in enumerate(b):
    yasn= splitNumbers(getNumbers(co))
    sum+=yasn
print(sum)